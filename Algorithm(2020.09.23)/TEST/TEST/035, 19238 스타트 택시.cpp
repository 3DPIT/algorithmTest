...
int find_customer() {
	...
	while (!q.empty()) {
		...
		for (int d = 0; d < 4; d++) {
			...
			if (ny < 0 || ny >= N || nx < 0 || nx >= N
				|| board[ny][nx] == WALL || visit[ny][nx] == true) {
				continue;
			}
			visit[ny][nx] = true;
	...
	int ret = -1;
	int candi_val = 0x7fffffff;//최소값 뽑기 그것이 
